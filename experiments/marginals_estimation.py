...
def get_id_dicts():
    ...
    def plot_annotation_distribution(ANN, draw_lims, dim, savename, fontsize=20, figsize=(30,10)):
        store_ann = {'circle': dict.fromkeys(FEATS), 'square': dict.fromkeys(FEATS)}
        ...
        for cname in class_to_label_dict:
            ...
            for f_ind, feat in enumerate(FEATS):
                f = cANN[:, f_ind]
                store_ann[cname][feat] = f
        return store_ann
#    			ax = axs[clabel,f_ind]
...
#    
    ...
    def estimated_id_features_using_shap(SHAPS, ANNS, dim, draw_lims, id_savename, od_savename, fontsize=20, figsize=(30,15)):
        ...
        store_idf = {'circle': dict.fromkeys(FEATS), 'square': dict.fromkeys(FEATS)}
        for cname in classes:
        
        
           # cname = 'circle' ## Just so it is easier to work with
            ...
            for f, feat in enumerate(FEATS):
                F  = ann[:, f]
                S  = shap[:, f]
                idF  = F[S >= 0] ## So we take what here? All annotations that have a positive Shapley value? So it is idF that I want I think.
                store_idf[cname][feat] = idF
    ...
    os_dict = plot_annotation_distribution(os_ann, 
    ...
    for net_ind, net_name in enumerate(['VGG11']):
    	...
    	dict_uncal = estimated_id_features_using_shap(SHAP, ANNS, 128,
    	...
    	dict_xcal = estimated_id_features_using_shap(SHAP, ANNS, 128,
    	...
    	dict_mcal = estimated_id_features_using_shap(SHAP, ANNS, 128,
    		draw_limits, out_dir/('%s_2_mcal_id.png' % net_name),
    		out_dir/('%s_2_mcal_od.png' % net_name))
        
    return os_dict, dict_uncal, dict_xcal, dict_mcal